...
**Q3 — Slack:**
> Slack w pracy — używasz? (audit patrzy tylko w kanały w których jesteś, bez czytania treści)
> - Tak / Nie / Pomiń
...
Read `reference/activitywatch-api.md`. The health endpoint is `http://localhost:5600/api/0/info`. Run:
...
Three outcomes:
- **200 + JSON with `version` field:** AW server is up. Go to 2b (bucket check).
- **`Connection refused` / `Couldn't connect`:** AW not running. Go to 2c (launch).
- **`curl: command not found`:** system is missing curl (extremely unusual). Ask user to install curl and stop.

### 2b. Bucket health check
...
If that returns a bucket name, AW is fully healthy — say "ActivityWatch chodzi" and skip to Step 2d (browser extension check).
...
After install, go back to 2c (launch).
